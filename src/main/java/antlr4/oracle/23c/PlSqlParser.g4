...
unit_statement
    ...
    | alter_mle_env //区别：新增
    | alter_mle_module //区别：新增
    | alter_operator
    | alter_outline
    | alter_package
    | alter_pluggable_database //区别：新增
    | alter_pmem_filestore
    | alter_procedure
    | alter_profile //区别：新增
    | alter_property_graph //区别：新增
    | alter_resource_cost
    | alter_role
    ...
    | create_hierarchy
    | create_index
    | create_indextype //区别：新增
    | create_inmemory_join_group
    | create_java
    | create_json_relational_duality_view //区别：新增
    | create_library
    | create_lockdown_profile
    | create_materialized_view
    | create_materialized_view_log
    | create_materialized_zonemap
    | create_mle_env //区别：新增
    | create_mle_module //区别：新增
    | create_operator
    | create_outline
    | create_package
    | create_package_body
...
check_diskgroup_clause //区别：少了ALL
    : CHECK (REPAIR | NOREPAIR)? //inconsistent documentation
    ;
...
alter_domain
    : ALTER DOMAIN (IF EXISTS)? (schema_name '.')? domain_name=id_expression
    ( (ADD | MODIFY) DISPLAY display_expression=expression
    | DROP DISPLAY
    | (ADD | MODIFY) ORDER order_expression=expression
    | DROP ORDER
    )
    | annotations_clause
    ;
...
annotations_clause
    : ANNOTATIONS '(' (ADD | DROP) annotation_name=id_expression annotation_value=id_expression?
    (',' (ADD | DROP)? annotation_name=id_expression annotation_value=id_expression?)* ')'
    ;
...
alter_flashback_archive //区别：多了注释
...
alter_hierarchy //区别：多了IF EXISTS
    : ALTER HIERARCHY (IF EXISTS)? (schema_name '.')? hn=id_expression (RENAME TO nhn=id_expression | COMPILE)
    ;

alter_function //区别：多了IF EXISTS
...
alter_java //区别：多了IF EXISTS
    : ALTER JAVA (IF EXISTS)? (SOURCE | CLASS) (schema_name '.')? o=id_expression
...
create_function_body //区别：多了IF NOT EXISTS EDITIONALBLE
    : CREATE (OR REPLACE)? editionable_noneditionable? FUNCTION ( IF NOT EXISTS )? function_name ('(' parameter (',' parameter)* ')')?
...
accessor
    : unit_kind (schema_name '.')? id_expression
    ;
...
pipelined_clause
    : PIPELINED ( ( ROW | TABLE ) POLYMORPHIC ( USING implementation_package_name)? )?
    ;

sql_macro_clause
    : SQL_MACRO ('(' (TYPE INTRODUCTION_OP)? (SCALAR | TABLE) ')')?
    ;
...
alter_lockdown_profile //区别：少了(USERS '=' (ALL | COMMON | LOCAL))?
...
alter_pluggable_database
    : ALTER PLUGGABLE DATABASE pdb_name=id_expression?
    ( pdb_unplug_clause
...
pdb_unplug_clause
    : pdb_name=id_expression UNPLUG INTO CHAR_STRING pdb_unplug_encrypt?
    ;

pdb_unplug_encrypt
    : ENCRYPT USING transport_secret=CHAR_STRING
    ;

pdb_settings_clauses
    : (pdb_name=id_expression?
        ...
        | PRIORITY value=id_expression
        | SET CONTAINER_MAP EQUALS_OP map_object=CHAR_STRING
        )
      )
      | CONTAINERS ( DEFAULT TARGET EQUALS_OP ( '(' container_name=id_expression ')' | NONE )
                   | HOST EQUALS_OP hostname=CHAR_STRING
                   | PORT EQUALS_OP number=numeric
                   )
    ;
...
pdb_refresh_mode_clause
    : REFRESH MODE ( MANUAL | EVERY refresh_interval=numeric ( MINUTES  | HOURS ) | NONE )
    ;

pdb_refresh_switchover_clause
    : FROM dblink SWITCHOVER
    ;

pdb_datafile_clause
    : pdb_name=id_expression? DATAFILE ((filename | filenumber) (',' (filename | filenumber) )* | ALL) (ONLINE | OFFLINE)
    ;

pdb_recovery_clauses
    : pdb_name=id_expression? ( pdb_general_recovery
    | begin_or_end BACKUP
    | enable_or_disable RECOVERY
    )
    ;
...
pdb_change_state
    : pdb_name=id_expression? (pdb_open | pdb_close | pdb_save_or_discard_state)
    ;
...
instances_clause
    : INSTANCES EQUALS_OP ( ALL ( EXCEPT '(' CHAR_STRING (',' CHAR_STRING)* ')')? | CHAR_STRING (',' CHAR_STRING)*)
    ;
...
pdb_change_state_from_root
    : (pdb_name=id_expression (',' pdb_name=id_expression)* | ALL (EXCEPT (pdb_name=id_expression (',' pdb_name=id_expression)*))?)
    (pdb_open | pdb_close | pdb_save_or_discard_state)
    ;

application_clauses
    : APPLICATION ( app_name=id_expression (
    | BEGIN INSTALL CHAR_STRING ( COMMENT CHAR_STRING )?
    | END INSTALL CHAR_STRING?
    | BEGIN UPGRADE ( CHAR_STRING )? TO CHAR_STRING (COMMENT CHAR_STRING)?
    | END UPGRADE ( TO CHAR_STRING )?
    | BEGIN UNINSTALL
    | END UNINSTALL
    | SET PATCH numeric
    | SET VERSION CHAR_STRING
    | SET COMPATIBILITY VERSION ( CHAR_STRING | CURRENT )
    | SYNC TO ( CHAR_STRING | PATCH numeric )
    | ( app_name=id_expression (',' app_name=id_expression)*)? SYNC
    )
    | ALL (EXCEPT ( app_name=id_expression (',' app_name=id_expression)*))? SYNC
    )
    ;

snapshot_clauses
    : pdb_snapshot_clause | materialize_clause | create_snapshot_clause | drop_snapshot_clause | set_max_pdb_snapshots_clause
    ;

pdb_snapshot_clause
    : SNAPSHOT ( MANUAL
    | EVERY snap_interval=numeric ( MINUTES | HOURS )?
    | NONE
    )
    ;
...
create_snapshot_clause
    : SNAPSHOT snapshot_name=id_expression?
    ;

drop_snapshot_clause
    : DROP SNAPSHOT snapshot_name=id_expression?
    ;

set_max_pdb_snapshots_clause
    : SET MAX_PDB_SNAPSHOTS EQUALS_OP numeric
    ;

drop_mirror_copy
    : DROP MIRROR COPY mirror_name=id_expression
    ;
...
alter_type //区别：多了IF EXISTS 结构不一样
    : ALTER TYPE (IF EXISTS)? type_name
    (alter_type_clause | editionable_noneditionable)
//    (compile_type_clause
//    | replace_type_clause
...
alter_session //区别：PARALLEL不一样 多了
    : ALTER SESSION (
        ADVISE ( COMMIT | ROLLBACK | NOTHING )
        | CLOSE DATABASE LINK parameter_name
        | enable_or_disable COMMIT IN PROCEDURE
        | enable_or_disable GUARD
        | (enable_or_disable | FORCE) PARALLEL (DML | DDL | QUERY) (PARALLEL UNSIGNED_INTEGER)?
        | ENABLE RESUMABLE (TIMEOUT UNSIGNED_INTEGER)? (NAME identifier)?
        | DISABLE RESUMABLE
...
sequence_spec //区别：多了START WITH RESTART SCALE SHARD SESSION GLOBAL
...
create_analytic_view //区别：多了IF NOT EXISTS
    : CREATE (OR REPLACE)? (NOFORCE | FORCE)? ANALYTIC VIEW (IF NOT EXISTS)? av=id_expression
...
create_attribute_dimension //区别：多了IF NOT EXISTS
    : CREATE (OR REPLACE)? (NOFORCE | FORCE)? ATTRIBUTE DIMENSION (IF NOT EXISTS)? (schema_name '.')? ad=id_expression
        (SHARING '=' (METADATA | NONE))?
        classification_clause*
...
actions_clause // 区别：多了column
    : (object_action | ALL) ('(' column_name (',' column_name)* ')')? ON (DIRECTORY directory_name | (MINING MODEL)? (schema_name '.')? id_expression)
    | (system_action | ALL)
    ;
...
create_single_column_domain
    : CREATE DOMAIN (IF NOT EXISTS)? (schema_name '.')? domain_name=id_expression AS datatype
    ( column_properties_clause
    | DISPLAY display_expression=expression
    | ORDER order_expression=expression
    | annotations_clause
    )*
    ;
...
column_properties_clause
    ...
    | NOT? NULL_
    | constraint_clause
    | VALIDATE USING schema_constant_text=CHAR_STRING
    | COLLATE column_collation_name
    ;

default_clause
    : DEFAULT ( ON NULL_ ( FOR INSERT ( ONLY | AND UPDATE ) )?)? default_expression=expression
    ;

constraint_clause
    : (
    CONSTRAINT constraint_name? ( NOT? NULL_ | CHECK condition )? constraint_state
    )+
    ;

create_multi_column_domain
    : CREATE DOMAIN ( IF NOT EXISTS )? (schema_name '.')? domain_name=id_expression AS
    ( '(' column_name AS datatype column_properties_clause (',' column_properties_clause)* ')'
    | DISPLAY display_expression=expression
    | ORDER order_expression=expression
    | annotations_clause
    )?
...
create_flexible_domain
    : CREATE FLEXIBLE DOMAIN ( IF NOT EXISTS )? (schema_name '.')? domain_name=id_expression
    '(' column_name ( ',' column_name)* ')'
    CHOOSE DOMAIN USING '(' column_name datatype( ',' datatype)* ')'
    FROM
    ( DECODE expr=expression ',' search_expr=expression ',' result_expr=expression ( ',' search_expr=expression (',' result_expr=expression )+ )*
    | CASE ( expr=expression (WHEN comparison_expr=expression THEN return_expr )+ | ( WHEN condition THEN return_expr)+ )
    ( ELSE else_expr=expression )? END
    )
    ;

return_expr
    : (schema_name '.')? domain_name=id_expression '(' column_name ( ',' column_name)* ')'
    ;

// https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/CREATE-EDITION.html
create_edition //区别：多了IF NOT EXISTS
    : CREATE EDITION (IF NOT EXISTS)? e=id_expression (AS CHILD OF pe=id_expression)?
    ;
...
create_hierarchy //区别：多了IF NOT EXISTS
    : CREATE (OR REPLACE)? (NO? FORCE)? HIERARCHY (IF NOT EXISTS)? (schema_name '.')? h=id_expression
        (SHARING '=' (METADATA | NONE))?
        classification_clause*
        hier_using_clause
        level_hier_clause
...
create_indextype
    : CREATE (OR REPLACE)? INDEXTYPE (IF NOT EXISTS)? indextype
        (SHARING '=' ( METADATA | NONE ))?
        FOR (schema_name '.')? operator_name '(' datatype (',' datatype)* ')'
            (',' (schema_name '.')? operator_name '(' datatype (',' datatype)* ')')*
        using_type_clause
        (WITH LOCAL RANGE? PARTITION)?
        storage_table_clause?
    ;

create_index //区别：多了IF NOT EXISTS index_ilm_clause
    : CREATE (UNIQUE | BITMAP)? INDEX (IF NOT EXISTS)? index_name index_ilm_clause?
...
index_properties
    ...
    | INDEXTYPE IS (domain_index_clause | xmlindex_clause | annotations_clause)
    ;
...
index_ilm_clause
    : ILM ((ADD POLICY)? policy_clause
    | DELETE POLICY policy_name)
    ;

policy_clause
    : (OPTIMIZE condition_clause)?
    | tiering_clause function_name?
    ;
...
tracking_statistics_clause
    : AFTER time_interval=numeric (DAYS | MONTHS | YEARS) OF NO? (ACCESS | MODIFICATION | CREATION)
    ;

alter_index_ops_set1 //区别：多了partial_index_clause
...
rebuild_clause //区别：多了partial_index_clause
...
modify_index_default_attrs //区别：多了+
...
modify_index_partition //区别：多了CLEANUP? ONLY? parallel_clause?
...
index_partition_description //区别：多了USABLE
...
alter_indextype //parameter_types不确定
    : ALTER INDEXTYPE (IF EXISTS)? (schema_name '.')? indextype
    ( (ADD | DROP) (schema_name '.')? operator_name '(' datatype (',' datatype)* ')'
    (',' (ADD | DROP) (schema_name '.')? operator_name '(' datatype (',' datatype)* ')')* using_type_clause?
    | COMPILE
    )
...
using_type_clause
    : USING (schema_name '.')? implementation_type_name array_DML_clause?
    ;

array_DML_clause
    : (WITH | WITHOUT)? ARRAY DML
    ('(' (schema_name '.')? type=id_expression (',' (schema_name '.')? varray_type=id_expression) ')'
    ( ',' '(' (schema_name '.')? type=id_expression (',' (schema_name '.')? varray_type=id_expression) ')')*
    )?
    ;

// https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/ALTER-INMEMORY-JOIN-GROUP.html
alter_inmemory_join_group //区别：多了IF EXISTS
    : ALTER INMEMORY JOIN GROUP (IF EXISTS)? (schema_name '.')? jg=id_expression
...
alter_user //区别：多了IF EXISTS NO AUTHENTICATION
...
isolate_keystore
    : ISOLATE KEYSTORE IDENTIFIED BY i=keystore_password FROM ROOT KEYSTORE
        force_keystore?
        identified_by_store
        with_backup_clause?
    ;

unite_keystore// 区别：多了 FORCE?
...
set_key // 区别：多了 ((mkid ':')? mk)?
    : SET ENCRYPTION? KEY using_tag_clause?
...
add_update_secret // 区别 identified_by_store是必有的
...
delete_secret_seps // 区别：多了 SQ
    : DELETE SECRET s=CHAR_STRING FOR CLIENT ci=CHAR_STRING
        FROM LOCAL? AUTO_LOGIN KEYSTORE directory_path
    ;

zero_downtime_software_patching_clauses // 区别：多了 TO
...
create_directory //区别：多了IF NOT EXISTS
...
create_inmemory_join_group //区别：多了IF NOT EXISTS
    : CREATE INMEMORY JOIN GROUP (IF NOT EXISTS)? (schema_name '.')? jg=id_expression
...
alter_library //区别：多了IF EXISTS
    : ALTER LIBRARY (IF EXISTS)? library_name
...
create_java //区别：多了IF NOT EXISTS
    : CREATE (OR REPLACE)? (AND (RESOLVE | COMPILE))? NOFORCE?
        JAVA (IF NOT EXISTS)? ( (SOURCE | RESOURCE) NAMED (schema_name '.')? pn=id_expression
...
create_json_relational_duality_view
    :  CREATE (OR REPLACE)?  ( NO? FORCE )? editionable_noneditionable?
    JSON RELATIONAL? DUALITY VIEW tableview_name AS
    ( SELECT object_gen_clause FROM table_name table_alias? table_tags_clause?
    | graphql_query_for_DV
    )
    ;

object_gen_clause
    : JSON LEFT_BRACE key_value_clause (',' key_value_clause)* RIGHT_BRACE
    ;

key_value_clause
    : KEY? CHAR_STRING (':' | IS) (column_name column_tags_clause? | '[' duality_view_subquery ']' | '(' duality_view_subquery ')')
    | UNNEST '(' duality_view_subquery ')'
    ;

column_tags_clause
    : WITH ((CHECK | NOCHECK) ETAG? | UPDATE | NOUPDATE)*
    ;

duality_view_subquery
    : SELECT object_gen_clause FROM table_name table_alias? table_tags_clause? WHERE join_condition
    ;

table_tags_clause
    : WITH ((CHECK | NOCHECK) ETAG? | INSERT | NOINSERT | UPDATE | NOUPDATE | DELETE | NODELETE)*
    ;
...
root_query_field
    : id_expression directives? selectionSet
    ;
...
directive
    : '@' id_expression '(' (id_expression ':' parameter_value) ')'?
    ;
...
create_library //区别：多了IF NOT EXISTS
    : CREATE (OR REPLACE)? editionable_noneditionable? LIBRARY (IF NOT EXISTS)? plsql_library_source
    ;
...
alter_dimension //区别：多了schema
    : ALTER DIMENSION (schema_name '.')? identifier
...
alter_view //区别：多了IF EXISTS annotations_clause
    : ALTER VIEW (IF EXISTS)? tableview_name
...
alter_tablespace //区别：多了IF EXISTS alter_tablespace_encryption lost_write_protection
    : ALTER TABLESPACE tablespace (IF EXISTS)?
       ( DEFAULT table_compression? default_index_compression? inmemory_clause? ilm_clause? storage_clause?
...
inmemory_clause
    : (INMEMORY inmemory_attributes? ( TEXT (column_list | column_name USING policy_name (',' column_name USING policy_name )* ))? | NO INMEMORY)
    ;
...
advanced_index_compression
    : COMPRESS ADVANCED low_high | NOCOMPRESS
    ;
...
alter_materialized_view //区别：多了IF EXISTS alter_table_partitioning evaluation_edition_clause
    : ALTER MATERIALIZED VIEW (IF EXISTS)? tableview_name
       ...
       (enable_or_disable ON QUERY COMPUTATION)?
       ( enable_or_disable QUERY REWRITE unusable_editions_clause?
       | enable_or_disable CONCURRENT REFRESH
       | COMPILE
       | CONSIDER FRESH
       )?
    ;
...
alter_materialized_view_log //区别：多了IF EXISTS
    : ALTER MATERIALIZED VIEW LOG (IF EXISTS)? FORCE? ON tableview_name
       ( physical_attributes_clause
...
for_refresh_clause
    : FOR ( SYNCHRONOUS REFRESH USING staging_log_name=id_expression
    | FAST REFRESH
    )
    ;
...
create_materialized_view_log //区别：多了IF NOT EXISTS
    : CREATE MATERIALIZED VIEW LOG (IF NOT EXISTS)? ON tableview_name
        sharing_clause?
        ( physical_attributes_clause
...
create_materialized_zonemap //区别：多了IF NOT EXISTS
    : CREATE MATERIALIZED ZONEMAP (IF NOT EXISTS)? zonemap_name (LEFT_PAREN column_list RIGHT_PAREN)? zonemap_attributes? zonemap_refresh_clause?
        ((ENABLE | DISABLE) PRUNING)? (create_zonemap_on_table | create_zonemap_as_subquery)
    ;

alter_materialized_zonemap //区别：多了IF EXISTS
    : ALTER MATERIALIZED ZONEMAP (IF EXISTS)? zonemap_name
...
zonemap_attributes //区别：少了SCALE TABLESPACE
    : (PCTFREE numeric | PCTUSED numeric | (CACHE | NOCACHE))+
    ;
...
create_mle_env
    : CREATE (OR REPLACE)? MLE ENV (IF NOT EXISTS)? (schema_name '.')? name=id_expression
    ( CLONE (schema_name '.')? environment_name=id_expression
    | (IMPORTS '(' CHAR_STRING MODULE (schema_name '.')? mle_module_name (',' CHAR_STRING MODULE (schema_name '.')? mle_module_name)* ')')?
    ( LANGUAGE OPTIONS CHAR_STRING )?
    )
    ;

alter_mle_env
    : ALTER MLE ENV (IF EXISTS)? (schema_name '.')? name=id_expression
    ( ADD IMPORTS '(' (import_name MODULE (schema_name '.')? mle_module_name)(',' import_name MODULE (schema_name '.')? mle_module_name)* ')'
    | DROP IMPORTS '(' (import_name)(',' import_name)* ')'
    | ALTER IMPORTS '(' (import_name MODULE (schema_name '.')? mle_module_name)(',' import_name MODULE (schema_name '.')? mle_module_name)* ')'
    | SET LANGUAGE OPTIONS option_string=CHAR_STRING
    | COMPILE
    )
    ;
...
create_mle_module
    ...
    (USING BFILE '(' id_expression ',' id_expression ')'
    | (CLOB | BLOB | BFILE) select_only_statement
//    | AS module_text=CHAR_STRING //module_text TODO JavaScript
    )
    ;

alter_mle_module //clob?
    : ALTER MLE MODULE (IF EXISTS)? (schema_name '.')? module_name=id_expression
...
create_materialized_view //区别：多了IF NOT EXISTS
    : CREATE MATERIALIZED VIEW (IF NOT EXISTS)? tableview_name
      (OF type_name )?
        ( '(' mv_column_alias (scoped_table_ref_constraint | annotations_clause)? (',' mv_column_alias (scoped_table_ref_constraint | annotations_clause)?)* ')' )?
        annotations_clause? (DEFAULT COLLATION collation_name)?
        ( ON PREBUILT TABLE ( (WITH | WITHOUT) REDUCED PRECISION)?
        | physical_properties? column_properties? table_partitioning_clauses? (CACHE | NOCACHE)? parallel_clause? build_clause?
        )
        ( USING INDEX ( (physical_attributes_clause | TABLESPACE mv_tablespace=id_expression)+ )*
        | USING NO INDEX
        )?
        create_mv_refresh? evaluation_edition_clause?
        ( (DISABLE | ENABLE) QUERY COMPUTATION )?
...
create_cluster //区别：多了IF NOT EXISTS
    : CREATE CLUSTER (IF NOT EXISTS)? cluster_name '(' column_name datatype SORT? (',' column_name datatype SORT?)* ')'
          ( physical_attributes_clause
          | SIZE size_clause
          | TABLESPACE tablespace
          | INDEX
...
create_table //区别：多了IF NOT EXISTS
    : CREATE
...
physical_attributes_clause //区别：少了
...
segment_attributes_clause //区别：少了table_compression
    : ( physical_attributes_clause
...
alter_system
    ...
    | FLUSH ( SHARED_POOL | GLOBAL CONTEXT | BUFFER_CACHE | FLASH_CACHE | REDO TO target_db_name=id_expression ( NO? CONFIRM APPLY )? )
    | end_session_clauses
    | SWITCH LOGFILE
    | ( SUSPEND | RESUME )
    | quiesce_clauses
    ...
    | RELOCATE CLIENT client_id=id_expression
    | cancel_sql_clause
...
archive_log_clause
    : ARCHIVE LOG (INSTANCE instance_name=CHAR_STRING)?
...
end_session_clauses
    : ( DISCONNECT SESSION CHAR_STRING POST_TRANSACTION?
    | KILL SESSION CHAR_STRING
    ) (IMMEDIATE | FORCE | NOREPLAY)? (TIMEOUT UNSIGNED_INTEGER)?
    ;
...
rolling_migration_clauses
    : START ROLLING MIGRATION TO CHAR_STRING
...
security_clauses
    ...
    | SET ENCRYPTION WALLET OPEN IDENTIFIED BY password_value
    | SET ENCRYPTION WALLET CLOSE (IDENTIFIED BY password_value)?
    | set_encryption_key
    ;

set_encryption_key
    : SET ENCRYPTION KEY (CHAR_STRING? IDENTIFIED BY password_value
    | IDENTIFIED BY password_value (MIGRATE USING password_value)?
    )
    ;

affinity_clauses
    : ENABLE AFFINITY tableview_name (SERVICE service_name=id_expression)?
    | DISABLE AFFINITY tableview_name
    ;

shutdown_dispatcher_clause
    : SHUTDOWN IMMEDIATE? dispatcher_name=id_expression
    ;

alter_system_set_clause
    : set_parameter_clause
    | USE_STORED_OUTLINES EQUALS_OP (TRUE | FALSE | id_expression)
    | GLOBAL_TOPIC_ENABLED EQUALS_OP (TRUE | FALSE)
    ;

set_parameter_clause
    : parameter_name EQUALS_OP parameter_value (',' parameter_value)? ( COMMENT EQUALS_OP quoted_string )? DEFERRED?
    ( CONTAINER EQUALS_OP (CURRENT | ALL) )? ( SCOPE EQUALS_OP (MEMORY | SPFILE | BOTH) | SID EQUALS_OP CHAR_STRING )*
    ;

alter_system_reset_clause
    : parameter_name ( SCOPE EQUALS_OP (MEMORY | SPFILE | BOTH) | SID EQUALS_OP CHAR_STRING )*
    ;

cancel_sql_clause
    : CANCEL SQL CHAR_STRING
    ;
...
alter_analytic_view // 区别：多了IF EXISTS
    : ALTER ANALYTIC VIEW (IF EXISTS)? (schema_name '.')? av=id_expression
...
alter_attribute_dimension // 区别：多了IF EXISTS
    : ALTER ATTRIBUTE DIMENSION (IF EXISTS)? (schema_name '.')? ad=id_expression
...
alter_audit_policy // 区别：add drop语句
...
alter_cluster //区别：多了IF EXISTS MODIFY PARTITION
    : ALTER CLUSTER (IF EXISTS)? cluster_name
        ( physical_attributes_clause
        | SIZE size_clause
        | (MODIFY PARTITION partition_name)? allocate_extent_clause
        | deallocate_unused_clause
        | cache_or_nocache
        )+
        parallel_clause?
...
partial_database_recovery //区别：注释10g
    : TABLESPACE tablespace (',' tablespace)*
...
managed_standby_recovery //区别：ARCHIVED LOGFILE 多了USING INSTANCES
    : RECOVER (MANAGED STANDBY DATABASE
               ((USING ARCHIVED LOGFILE
                ...
                | UNTIL CONSISTENT
                | USING INSTANCES (ALL | UNSIGNED_INTEGER)?
                | parallel_clause
                )+
               | FINISH
               | CANCEL
...
create_datafile_clause //区别：处理了file_specification
    : CREATE DATAFILE (filename | filenumber) (',' (filename | filenumber) )*
        (AS ( file_specification (',' file_specification)* | NEW) )?
    ;
...
add_logfile_clauses //区别：处理了group_redo_logfile
    : ADD STANDBY? LOGFILE
        ( (INSTANCE CHAR_STRING | THREAD UNSIGNED_INTEGER)? group_redo_logfile (',' group_redo_logfile)*
...
supplemental_db_logging//区别：多了supplemental_subset_replication_clause
    :  add_or_drop
...
controlfile_clauses //多了FAR SYNC INSTANCE
...
standby_database_clauses //区别：多了switchover_clause | failover_clause
...
register_logfile_clause //区别：处理了file_specification for logminer_session_name
    : REGISTER (OR REPLACE)? (PHYSICAL | LOGICAL) LOGFILE file_specification (',' file_specification)*
        (FOR logminer_session_name=id_expression)?
    ;

commit_switchover_clause //区别：bug LOGICAL STANDBY
...
switchover_clause
    : SWITCHOVER TO target_db_name=id_expression ( VERIFY | FORCE )?
    ;

failover_clause
    : FAILOVER TO target_db_name=id_expression FORCE?
    ;

default_settings_clause //区别：多了LOCAL
    : DEFAULT EDITION EQUALS_OP edition_name
    | SET DEFAULT (BIGFILE | SMALLFILE) TABLESPACE
    | DEFAULT TABLESPACE tablespace
...
alter_database_dictionary //区别：新增
    : ALTER DATABASE DICTIONARY (ENCRYPT CREDENTIALS | REKEY CREDENTIALS | DELETE CREDENTIALS KEY)
    ;

alter_database_link //区别：多了IF EXISTS
    : ALTER SHARED? PUBLIC? DATABASE LINK (IF EXISTS)? link_name (CONNECT TO user_object_name IDENTIFIED BY password_value link_authentication? | link_authentication)
    ;
...
// added by zrh
create_database //区别：多了USING MIRROR COPY
    ...
    ( USER (SYS | SYSTEM) IDENTIFIED BY password_value
     ...
     | database_name USING MIRROR COPY mirror_name=id_expression
    )+
    ;
...
create_database_link //区别：多了IF NOT EXISTS
    : CREATE SHARED? PUBLIC? DATABASE LINK (IF NOT EXISTS)? dblink ( CONNECT TO ( CURRENT_USER
...
modify_to_partitioned
    : MODIFY table_partitioning_clauses
        filter_condition?
        ONLINE?
        ( UPDATE INDEXES ( '(' index_name ( local_partitioned_index | global_partitioned_index | GLOBAL )
                           ( ',' index_name ( local_partitioned_index | global_partitioned_index | GLOBAL ) )* ')'
                         )?
        )?
    ;
...
modify_opaque_type
    : MODIFY OPAQUE TYPE anydata_column=id_expression STORE ( type_name ( ',' type_name )* ) UNPACKED
    ;

alter_table_properties
    : alter_table_properties_1
    | RENAME TO tableview_name alter_iot_clauses? alter_XMLSchema_clause?
    | shrink_clause
    | READ ONLY
    | READ WRITE
    | REKEY CHAR_STRING
...
alter_table_properties_1 //区别：多了ilm_clause
    : ( physical_attributes_clause
      | logging_clause
      | table_compression
      | inmemory_table_clause
...
query_block //区别：from_clause
    : SELECT (DISTINCT | UNIQUE | ALL)? selected_list
      into_clause? from_clause? where_clause? hierarchical_query_clause? group_by_clause? model_clause? window_clause? order_by_clause? fetch_clause? row_limiting_clause?
    ;
...
inline_analytic_view
    : ANALYTIC VIEW sub_av_clause (AS? table_alias)?
    ;

sub_av_clause
    : USING (schema_name '.')? ban=id_expression hierarchies_clause filter_clauses? add_means_clause?
    ;

hierarchies_clause
    : HIERARCHIES '(' ((table_alias '.')? column_alias (',' (table_alias '.')? column_alias)*)? ')'
    ;
...
hier_id
    : MEASURES | table_alias '.' column_alias
    ;

add_means_clause
    : ADD MEASURES '(' cube_means (',' cube_means)* ')'
    ;

cube_means
    : means_name=id_expression (base_meas_clause | calc_meas_clause)
    ;
...
window_clause
    : WINDOW (window_name=id_expression AS window_specification (',' window_name=id_expression AS window_specification)*)?
    ;

window_specification
    : existing_window_name=id_expression? query_partition_clause? order_by_clause? windowing_clause?
    ;

row_limiting_clause
    : (OFFSET numeric (ROW | ROWS))? (FETCH (FIRST | NEXT) (numeric PERCENT?)? (ROW | ROWS) (ONLY | WITH TIES))
    | (OFFSET numeric (ROW | ROWS)) (FETCH (FIRST | NEXT) (numeric PERCENT?)? (ROW | ROWS) (ONLY | WITH TIES))?
    ;

update_statement
    : UPDATE general_table_ref update_set_clause from_clause? where_clause? order_by_clause? static_returning_clause? error_logging_clause?
    ;
...
delete_statement
    : DELETE FROM? general_table_ref from_using_clause? where_clause? static_returning_clause? error_logging_clause?
    ;

from_using_clause
    : (FROM | USING) (table_ref_list | inline_analytic_view (',' inline_analytic_view)*)
    ;
...
merge_statement
    : MERGE INTO tableview_name table_alias?
    (USING selected_tableview ON '(' condition ')'
    | values_clause ON '(' condition ')'
    )
